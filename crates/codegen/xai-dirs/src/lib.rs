...
/// True when `path` is on Android shared storage (FAT/FUSE `/sdcard`, `/storage/emulated/0`, …).
/// Those filesystems do not enforce POSIX 0700, so credentials must never live there.
fn is_android_shared_storage(path: &Path) -> bool {
    ...
    dunce::canonicalize(path)
        .map(|canon| is_android_shared_storage_str(&canon.to_string_lossy().replace('\\', "/")))
        .unwrap_or(false)
}

fn is_android_shared_storage_str(lower: &str) -> bool {
...
/// Android shared-storage overrides are rejected and fall through to the home default.
fn resolve_grok_home_from(
    ...
    if let Some(env) = grok_home_env.filter(|env| !env.is_empty()) {
        let override_path = PathBuf::from(env);
        if is_android_shared_storage(&override_path) {
            tracing::error!(
                path = %override_path.display(),
                "Rejected insecure GROK_HOME location on Android shared storage; falling back to default"
            );
        } else {
            return Some((override_path, GrokHomeSource::EnvOverride));
        }
    }
...
mod tests {
    ...
    #[test]
    fn shared_storage_grok_home_falls_back_to_os_home() {
        let tmp = tempfile::tempdir().unwrap();
        let resolved = resolve_grok_home_from(
            Some(OsStr::new("/storage/emulated/0/.grok")),
            Some(tmp.path()),
        );
        assert_eq!(
            resolved,
    ...
    #[test]
    fn sdcard_grok_home_is_rejected() {
        assert!(is_android_shared_storage(Path::new("/sdcard/.grok")));
        assert!(is_android_shared_storage(Path::new(
            "/storage/emulated/0/Download/.grok"
        )));
        assert!(!is_android_shared_storage(Path::new(
