...
pub async fn get_installer() -> Option<&'static str> {
    ...
    match cfg.cli.installer.as_deref() {
        Some("npm") => return Some("npm"),
        Some("gh-release") => return Some("gh-release"),
        Some("pkg") | Some("package-managed") | Some("apt") => return Some("package-managed"),
        Some("standalone") | Some("internal") => {}
        Some(_) => return Some("internal"),
        // A wiped config must not reclassify an npm install as internal:
        // that re-enables downgrades and updates npm never sees.
        None if path_resolves_to_npm_entry() => return Some("npm"),
        None => {}
    }
            ...
            if offset + phentsize <= bytes.len() {
                ...
                if p_type == 3 {
                    ...
                    if p_offset + p_filesz <= bytes.len() {
                        ...
                        let interp_str = String::from_utf8_lossy(interp_bytes)
                            .trim_matches('\0')
                            .to_string();
                        if is_android
                            && (interp_str.contains("ld-linux") || interp_str.starts_with("/lib"))
                        {
                            anyhow::bail!(
