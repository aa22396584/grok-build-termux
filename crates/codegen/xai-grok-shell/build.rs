...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Returning before `cargo:rustc-cfg=bundle_rg` keeps the include_bytes! macros compiled out. The runtime then falls back to `rg` on PATH.
    // Android/Termux also skips bundling and resolves native `rg` from $PATH / $PREFIX/bin.
    // An explicit GROK_SHELL_BUNDLE_RG_PATH still bundles regardless of target.
